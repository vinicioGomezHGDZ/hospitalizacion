angular.module("brander",['ngRoute'])
.controller('branderCtrl',['$scope','$http','$routeParams',function($scope,$http,$routeParams,ModalService){
     ...
     if (data != "0")
      {  
         $scope.datosAguardar={
             ...
             bra_fecha:new Date(data[0].bra_fecha)
        }
         $("#n").click()
      }else
      {
...
$scope.actualizar=function(){
  ...
  $http.post('src/sgh/brander/php/sghInserBrander.php',{op:2,tra:$scope.datosAguardar, usu:$scope.usuario}).success(function(data){
       $scope.text = data.sgh_brander_ingreso_pa;
       $scope.mensaje= true;
