...
# CARGAR datos generales anamnesis
  ...
  per_nombres || ' ' ||per_apellidopaterno ||' '|| per_apellidomaterno as profecional, pr.pro_codigomsp,ana_hora",
    ...
    # CARGAR DIAGNOSTICO  
        $dia=$ConDI->Get_Consulta(
			"sgh_mei_danamdiag join sgh_mei_diagnos as d on dia_id_fk=d.dia_id_pk JOIN sgh_mei_codigocie10 as c10  on d.c10_id_fk=c10.c10_id_pk 
			 where ana_id_fk=".$ana_id_pk,
			" c10_nombre AS detalle,
			c10_codigo as cie,
...
<!-- 2 hoja  -->	
	...
	<table  border="1" width="100%" cellspacing="0" cellpadding="2">
        ...
        <tr>
						<td height="380" VALIGN="TOP" colspan=15>
	...
	<table><tr><td></td></tr></table>
			...
			<table border="1" width="100%" cellspacing="0" cellpadding="2">
					...
					for ($i=0; $i < count($dia); $i++) {
						$it=$i+1;
						$html .='<tr>
         						<td widt="1"><font size=1>'.($it).'</font></td>
								<td widt="500"><font size=1>'.($dia[$i]["detalle"] ?? '' ).'</font></td>
	...
	<table border="1" cellspacing="0" width="100%"> 
		<tr>
			...
			<td width="10"><center><font size=1>'.(substr($ana[0]["ana_hora"],0, -7)  ?? '' ).'</font></center></td>
