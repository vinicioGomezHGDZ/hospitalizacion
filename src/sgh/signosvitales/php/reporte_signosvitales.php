...
// CONSULTAS DE REPORRTE
	# CARGAR DATOS DE ENCABEZADO. array 0
		...
		JOIN sga_adm_sexo as se on se.sex_id_pk=per.sex_id_fk","per_nombres,(per_apellidopaterno ||' '|| per_apellidomaterno ) as apellido,
			date_part('year',age(per_fechanacimiento)) as Edad,hce_numerohc,sex_codigo as per_sexo,per_numeroidentificacion","","hce_id_fk",$hce_id_pk,2);
			//print_r($per);	
...
join sga_adm_persona per on pr.per_id_fk=per.per_id_pk where svd_fecha>='".$fecha."' and hce_id_fk='".$hce_id_pk."' order by svd_fecha desc","svd_fecha,svd_hora,svd_diante,svd_pulso,svd_tempe,svd_freres,svd_presis,svd_predia,svd_satoxi,svd_parent,svd_viaora,svd_toteli,svd_orina,svd_drenaj,svd_otros,svd_toting,
...
join sgu_usu_usuario us on usu_id_fk=usu_id_pk
join sga_adm_profesional pr on us.pro_id_fk=pro_id_pk
join sga_adm_persona per on pr.per_id_fk=per.per_id_pk where svd_fecha>='".$fecha."' and svd_dieadm is not null AND hce_id_fk='".$hce_id_pk."' order by svd_fecha desc","svd_fecha,svd_hora,svd_diante,svd_pulso,svd_tempe,svd_freres,svd_presis,svd_predia,svd_satoxi,svd_parent,svd_viaora,svd_toteli,svd_orina,svd_drenaj,svd_otros,svd_toting,
 CASE WHEN svd_aseo=true then 'X' end aseo,
 CASE WHEN svd_banio=true then 'X' end baño
,svd_peso,svd_dieadm,svd_numcom,svd_numicc,svd_numdep,svd_actfis,
CASE WHEN svd_camson=true then 'X' end sonda,svd_recvia,
per_nombres || ' ' || per_apellidopaterno || ' ' || per_apellidomaterno as responsable,svd_talla","","","",5);
...
<!-- diseño encabezado pie de pagina -->		
	...
	<table width="100%" >
	    ...
	    <td width="20%" align="center" style="font-weight: bold; font-style: italic;"></td>
...
<!-- primera hoja  -->	
	...
	<table border=1 cellspacing="0" cellpadding="2" width="100%">
		...
		<tr>
			...
			<th class="" width=30><font size=1>DÍA DE INTER-<br>NACACIÓN </font> </th>
	...
	<table border=1 cellspacing="0" cellpadding="2" width="100%">
		...
		<tr>
			...
			<td class="">
				<table width="100%" border=1 cellspacing="0" cellpadding="2">
					<tr>
						...
						<th width=30>
							<font size=1>DÍA DE INTER-<br>NCACIÓN</font>
						</th>
						<th  width=30>
			...
			<td align="center"><font size=1>'.($sivg[$i]["responsable" ] ?? '').'</font> </td>
			</tr>
			';
		}
		$html.='
			...
			<td class="">
				<table width="100%" border=1 cellspacing="0" cellpadding="2">
					<tr>
						...
						<th width=30>
							<font size=1>DÍA DE INTER-<br>NACACIÓN</font>
						</th>
						<th>
	...
	<table><tr><td></td></tr></table>
		...
		<tr>
			...
			<tr>
						...
						<th width=30>
							<font size=1>DÍA DE INTE-<br>RNACACIÓN</font>
						</th>
						<th>
				...
				';
					for ($i=0; $i <count($sivg) ; $i++) { 
						...
						<td align="center"><font size=1>'.($sivg[$i]["svd_talla" ] ?? '').'</font> </td>
