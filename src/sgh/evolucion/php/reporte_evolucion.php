...
# CARGAR DATOS DE ENCABEZADO. array 0
	$per=$Con->Get_Consulta("  sgh_mei_evolucion ep
		...
		JOIN sga_adm_sexo as se on se.sex_id_pk=per.sex_id_fk","DISTINCT per_nombres,(per_apellidopaterno ||' '|| per_apellidomaterno ) as apellido,
         ...
         select to_char(eyp_fechas,'dd-MM-YYYY') as eyp_fechas,eyp_hora,regexp_replace(eyp_nodevu,'\n','<br>','g') eyp_nodevu,
          ...
          from sgh_mei_evolucion ev
           ...
           JOIN sga_adm_profesional as pr on us.pro_id_fk=pro_id_pk
			...
			JOIN sga_adm_profesion as pro on pr.prf_id_fk = pro.prf_id_pk where eyp_fechas>='".$fecha."' and hce_id_fk='".$hce_id_pk."' order by eyp_id_pk, eyp_hora
        )t");
