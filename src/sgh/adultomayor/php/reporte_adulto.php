...
// CONSULTAS DE REPORRTE
	# CARGAR DATOS DE ENCABEZADO. array 0
		...
		JOIN sga_adm_sexo as se on se.sex_id_pk=per.sex_id_fk","per_nombres,(per_apellidopaterno ||' '|| per_apellidomaterno ) as apellido,
			date_part('year',age(per_fechanacimiento)) as Edad,hce_numerohc,sex_codigo as per_sexo, per_numeroidentificacion","","aam_id_pk",$epi_id_pk,2);
	  	...
	  	<td VALIGN="TOP">	
			<table border="1" width="100%" cellspacing="0" cellpadding="2">
		    		<tr>
		    			...
		    			<th whdth="10" > <font size=1>P</font></th>  
		    			<th whdth="10" > <font size=1>D</font></th>
		    			<th whdth="20" > <font size=1>CIE</font></th>  
		    			<th whdth="70" > <font size=1>Clínico, Sindrómico, Psicologico, <br>Funcional, Nutricional </font></th>
					...
					for ($i=0; $i < 4; $i++) {
					    $dt=$i+1;
	            		$html .='<tr>
                            <td widt="1"><font size=1>'.$dt.'</font></td>
		    		   		<td widt=""><font size=1>'.($dia[$i]["detalle"] ?? '' ).'</font></td>
		    		   		<td widt=""><font size=1><center>'.($dia[$i]["pre"] ?? '' ).'</center></font></td>
		    		   		<td widt=""><font size=1><center>'.($dia[$i]["def"] ?? '' ).'</center></font></td>
		    		   		<td widt=""><font size=1><center>'.($dia[$i]["cie"] ?? '' ).'</center></font></td>

		    		   		<td widt=""><font size=1><center>'.($dia[$i]["dia_descrip"] ?? '' ).'</center></font></td>
