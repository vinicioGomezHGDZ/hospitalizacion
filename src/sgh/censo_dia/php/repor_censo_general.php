...
// total contar las defuncion de - de 48 horas
	$tdme=$Condme->Get_Consulta("sgh_mei_censo cen
        ...
        where tc.tca_descripcion ='".$serv."' and cen_def_48  = 't'  and cen_fecha='".$fecha."'","cen_fecha","","","",5);
...
// total contar las defuncion de - de 48 horas
        ...
        join sga_adm_cama as c on cen.cam_id_fk=c.cam_id_pk
		JOIN sga_adm_tipocama as tc on c.tca_serv_fk=tc.tca_id_pk
        where tc.tca_descripcion ='".$serv."' and cen_def48  = 't'  and cen_fecha='".$fecha."'","cen_fecha","","","",5);
...
//// total camas ocupadas
	$caoc=$Concaocu->Get_Consulta("sgh_mei_censo cen
		...
		join sga_adm_tipocama as ha on tca_habi_fk=ha.tca_id_pk where cen_tipo ='INGRESO' and tc.tca_descripcion ='".$serv."' and cen_visible=TRUE and cen_fecha<='".$fecha."'",
        "cen_fecha","","","",5);
...
// total camas servicio
	...
	WHERE ser.tca_descripcion='".$serv."' and cam_visible=true","ser.tca_descripcion,pis.tca_descripcion,ce.ces_descripcion","","","",5);
...
//// total camas dañada
	...
	JOIN sga_adm_tipocama ser on c.tca_serv_fk=ser.tca_id_pk
	JOIN sga_adm_tipocama pis on c.tca_piso_fk=pis.tca_id_pk
	WHERE ser.tca_descripcion='".$serv."' and ce.ces_descripcion='DAÑADA' and cam_visible=true ","ser.tca_descripcion,pis.tca_descripcion,ce.ces_descripcion ","","","",5);
...
// TOTAL A LAS 0 HORAS DEL MISMO DIA
   $t0hda=$Cont0hda->Get_Consulta("sgh_mei_censo cen
		...
		JOIN sga_adm_tipocama as tc on c.tca_serv_fk=tc.tca_id_pk
		join sga_adm_tipocama as pi on tca_piso_fk=pi.tca_id_pk
		join sga_adm_tipocama as ha on tca_habi_fk=ha.tca_id_pk where cen_tipo ='INGRESO' and tc.tca_descripcion ='".$serv."' and cen_visible=false and cen_fecha='".$fecha."'",
    "cen_fecha","","","",5);
...
# cargar datos de ingreso
		...
		JOIN sga_adm_tipocama as tc on c.tca_serv_fk=tc.tca_id_pk
		join sga_adm_tipocama as pi on tca_piso_fk=pi.tca_id_pk
		join sga_adm_tipocama as ha on tca_habi_fk=ha.tca_id_pk where cen_tipo ='INGRESO' and tc.tca_descripcion ='".$serv."' and  cen.cen_ingreso_tras is null and cen_fecha='".$fecha."'",
		"(per_nombres||' '||per_apellidopaterno ||' '|| per_apellidomaterno ) as persona,per_numeroidentificacion,hce_numerohc,hce_id_pk,
  		cam_codigo,tc.tca_descripcion as servicio,ha.tca_descripcion as habitacion ,pi.tca_descripcion as piso,cen_id_pk,cen_tipo,cen_ingreso_tras,cen_def_48,cen_def48,cen_fecha","","","",6);
...
# cargar datos de ingreso transferidos
		...
		JOIN sga_adm_tipocama as tc on c.tca_serv_fk=tc.tca_id_pk
		join sga_adm_tipocama as pi on tca_piso_fk=pi.tca_id_pk
		join sga_adm_tipocama as ha on tca_habi_fk=ha.tca_id_pk where cen_tipo ='INGRESO' and tc.tca_descripcion ='".$serv."' and  cen.cen_ingreso_tras='trans' and cen_fecha='".$fecha."' AND cen_visible=TRUE",
...
# cargar datos de egresos
		...
		JOIN sga_adm_tipocama as tc on c.tca_serv_fk=tc.tca_id_pk
		join sga_adm_tipocama as pi on tca_piso_fk=pi.tca_id_pk
		join sga_adm_tipocama as ha on tca_habi_fk=ha.tca_id_pk where cen_tipo ='EGRESO' and tc.tca_descripcion ='".$serv."' and  cen.cen_ingreso_tras is null and cen_fecha='".$fecha."'",

		"(per_nombres||' '||per_apellidopaterno ||' '|| per_apellidomaterno ) as persona,per_numeroidentificacion,
  		cam_codigo,tc.tca_descripcion as servicio,ha.tca_descripcion as habitacion ,pi.tca_descripcion as piso,cen_tipo","",
  		"","",6);

# cargar datos de egresos trans
		$etret=$Conet->Get_Consulta("sgh_mei_censo cen
        join sga_adm_historiaclinica his on cen.hce_id_fk=his.hce_id_pk
	 	join sga_adm_paciente as p on his.pac_id_fk=p.pac_id_pk
	 	join sga_adm_persona as pe on p.per_id_fk=pe.per_id_pk
        join sga_adm_cama as c on cen.cam_id_fk=c.cam_id_pk
		JOIN sga_adm_tipocama as tc on c.tca_serv_fk=tc.tca_id_pk
		join sga_adm_tipocama as pi on tca_piso_fk=pi.tca_id_pk
		join sga_adm_tipocama as ha on tca_habi_fk=ha.tca_id_pk where cen_tipo ='EGRESO' and tc.tca_descripcion ='".$serv."' and cen.cen_ingreso_tras='trans' and cen_fecha='".$fecha."'",
		"(per_nombres||' '||per_apellidopaterno ||' '|| per_apellidomaterno ) as persona,per_numeroidentificacion,hce_numerohc,hce_id_pk,
  		cam_codigo,tc.tca_descripcion as servicio,ha.tca_descripcion as habitacion ,pi.tca_descripcion as piso,cen_id_pk,cen_tipo,cen_ingreso_tras,cen_def_48,cen_def48,cen_fecha","",
  		"","",6);
        $totalegresost=sizeof($etret);
# cargar datos de defunciones
		...
		JOIN sga_adm_tipocama as tc on c.tca_serv_fk=tc.tca_id_pk
		join sga_adm_tipocama as pi on tca_piso_fk=pi.tca_id_pk
		join sga_adm_tipocama as ha on tca_habi_fk=ha.tca_id_pk where cen_tipo ='DEFUNCION' and tc.tca_descripcion ='".$serv."' and cen_fecha='".$fecha."'",
		"(per_nombres||' '||per_apellidopaterno ||' '|| per_apellidomaterno ) as persona,per_numeroidentificacion,hce_numerohc,hce_id_pk,
...
<!-- diseño encabezado pie de pagina -->		
	...
	<htmlpagefooter name="myFooter1" style="display:none">
	    ...
	    <td width="30%"><font size="3">M.S.P.-S1 - Form.555/88</font></td>

	    <td width="30%" align="center" style="font-weight: bold; font-style: italic;"></td>

	    <td width="40%" style="text-align: right; "><font size="3">CENSO DIARIO</font></td>
...
<table width="100%" >
	    ...
	    <td width="50%" style="text-align: right; "><h3>'.$Con->entidad.'</h3></td>
...
<table>
   <!-- superiors ingreso egresos-->
	<tr>
		...
		superior egresos y egreso transferencia-->
		<td VALIGN="TOP" width="500">	
			<table width="100%" border=1 cellspacing="0" cellpadding="3">
				<tr>
			...
			<table width="100%" border=1 cellspacing="0" cellpadding="3">
					...
					<tr>
						<td height="200" VALIGN="TOP" colspan="3">
							<table  width="100%">	
						 		...
						 		';
                                for ($i=0; $i < sizeof($etret); $i++) {
				            		$html .='<tr>
					    		   		<td widt=""><center> '.($etret[$i]["cam_codigo"] ?? '') .'</center></td>
					    		   		<td widt=""><center> '.($etret[$i]["per_numeroidentificacion"] ?? '') .'</center></td>
					    		   		<td widt=""><center> '.($etret[$i]["persona"] ?? '') .'</center></td>
					    				</tr>';	
				          		}
					    		$html .='
					...
					<tr>
					    			<th>
					    				Total de transferencias  otras especialidades
					    			</th>
					    			<td>| '.$totalegresost.' |</td>
