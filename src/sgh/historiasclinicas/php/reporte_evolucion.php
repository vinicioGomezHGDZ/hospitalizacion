...
$sql=$bdd->prepare("
        select json_agg(t) from (
         select to_char(eyp_fechas,'dd-MM-YYYY') as eyp_fechas,eyp_hora,regexp_replace(eyp_nodevu,'\n','<br>','g') eyp_nodevu,
          regexp_replace(eyp_prescr,'\n','<br>','g') eyp_prescr,eyp_asunto,pe.per_nombres || ' ' ||per_apellidopaterno  || ' ' || coalesce(per_apellidomaterno,'') as medico,' ' || pr.pro_codigomsp as msp,pr.pro_codigosenescyt as sncy,pe.per_numeroidentificacion as ci,eyp_revisaresidente
          from sgh_mei_evolucion ev
           ...
           JOIN sga_adm_profesional as pr on us.pro_id_fk=pro_id_pk
			...
			JOIN sga_adm_profesion as pro on pr.prf_id_fk = pro.prf_id_pk where  hce_id_fk='".$hce_id_pk."' order by eyp_id_pk,eyp_hora
        )t");
