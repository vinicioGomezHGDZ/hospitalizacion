...
$Conbra=New Consulta();
$hce_id_fk=$_GET['h'];
$brander=$Conbra->Get_Consulta("sgh_mei_branden where  hce_id_fk='".$hce_id_fk."' ORDER BY bra_fecha DESC","bra_id_pk,bra_fecha","","","",5);
...
for ($r=0; $r <count($brander) ; $r++) {
   ...
   $per=$$Con->Get_Consulta("sgh_mei_branden ep
    ...
    join sga_adm_tipocama as ser on ca.tca_serv_fk=ser.tca_id_pk","bra_id_pk,per_nombres || '' ||per_apellidopaterno ||' '|| coalesce(per_apellidomaterno,'')  as persona, cam_codigo as cama , tca_descripcion as servicio,per_numeroidentificacion","","bra_id_pk",$bra_id_pk,2);
# CARGAR DATOS DE ENCABEZADO. array 0
